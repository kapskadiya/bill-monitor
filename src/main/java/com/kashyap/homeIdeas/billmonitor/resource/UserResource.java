...
public class UserResource {
    ...
    public ResponseEntity<String> disableUser(@PathParam(value = "username") final String username) throws IOException {
        if (StringUtils.isBlank(username)) {
            return ResponseEntity
                    .badRequest()
                    .body("User can't be empty");
        }

        if (!userService.disableUser(username)) {
            return ResponseEntity
                    .status(HttpStatus.NOT_FOUND)
                    .body("user is not disable");

        }
        return ResponseEntity
                .ok()
                .body("user is disable");
    }
    ...
    public ResponseEntity<String> remove(@PathVariable final String username) throws IOException {

        final String result = userService.removeByUsername(username) ? "User is successfully deleted." : "User is not deleted.";
